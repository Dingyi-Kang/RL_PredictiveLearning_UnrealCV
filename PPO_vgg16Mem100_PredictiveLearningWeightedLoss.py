...
class Buffer:
    ...
    def __init__(self, observation_dimensions, size, gamma=0.99, lam=0.95):
        ...
        #if it is the begining of the tarjector, we mark it as 1. otherwise 0.
        self.trajectory_starts_buffer = np.zeros(size, dtype=np.int32)
    ...
    def finish_trajectory(self, last_value=0):
        ...
        #mark the begining of trajectory in the buffer
        self.trajectory_starts_buffer[self.trajectory_start_index] = 1
...
def create_cnn_predicted_attention(observation_dimensions):
    ...
    # Define input tensor for hidden state with an extra dimension for batch
    input_hidden_state = keras.Input(shape=(attention_shape, hidden_units))
    ...
    #print(previous_input_sequence.shape)

    # create RNN Predictor
    # Initial hidden state
    # Solved: reset the hidden state at the start fo each new episode
    # Remove: hidden_state = tf.Variable(tf.zeros((attention_shape, hidden_units)))
    hidden_state_with_time_axis = tf.reshape(input_hidden_state, (1, attention_shape, 512))
    #print(input_hidden_state.shape)
    #print(hidden_state_with_time_axis.shape)
    # Bahdanau Attention
    ...
    #Solved: Given we set the stateful_flag be false, we dont need to clean the LSTM anymore since there is hidden states pass between batch
    ...
    #MARK: 3. why?? prediction error, instead of (prediction minus previous) which is predicted suprise and then compare predicted suprise with actual suprise??
...
# Solved: be careful about this
@tf.function
def sample_action_and_train_prediction(observation, hidden_state):
    with tf.GradientTape() as tape:
        loss, lossGrid, new_hidden_state = cnnAttentionPredictor([observation, hidden_state])
    ...
    return logits, action, lossGrid, new_hidden_state

#if the frames are just duplicate, don't train prediction
def just_sample_action(observation, hidden_state):
    loss, lossGrid, new_hidden_state = cnnAttentionPredictor([observation, hidden_state])
    logits = actor(lossGrid)
    action = tf.squeeze(tf.random.categorical(logits, 1), axis=1)
    return logits, action, lossGrid, new_hidden_state
...
hidden_state = np.zeros((attention_shape, hidden_units))
# ------ train ------
...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        # Solved: pass hidden states here and need to intialize the hidden state and maintain/update it in the process. maybe reset after finish function?
        logits, action, lossGrid, new_hidden_state = None, None, None, None
        if duplicateFrames:
            logits, action, lossGrid, new_hidden_state = just_sample_action(reshaped_stacked_frames, hidden_state)
        else:
            logits, action, lossGrid, new_hidden_state = sample_action_and_train_prediction(reshaped_stacked_frames, hidden_state)
        ...
        # Update the observation and hidden state
        curr_observation = preprocess(observations[agentIndex])
        stacked_frames = stack_frames(stacked_frames, curr_observation, stack_size)
        ...
        if terminal or (t == steps_per_epoch - 1):
            ...
            if not done:
                ...
                logits, action, lossGrid, new_hidden_state = None, None, None, None
                if duplicateFrames:
                   _, _, lossGrid, _ = just_sample_action(reshaped_stacked_frames, hidden_state)
                else:
                   _, _, lossGrid, _ = sample_action_and_train_prediction(reshaped_stacked_frames, hidden_state) 
    ...
    hidden_state = np.zeros((attention_shape, hidden_units))
    #batch size is 1
    for obs_batch in get_batches(observation_buffer, 1):
        if trajectory_starts_buffer[trajectory_index] == 1:
            #reset hiddent states to zeros
            hidden_state = np.zeros((attention_shape, hidden_units))
        
        _, lossGrid_batch, new_hidden_state = cnnAttentionPredictor([obs_batch, hidden_state])

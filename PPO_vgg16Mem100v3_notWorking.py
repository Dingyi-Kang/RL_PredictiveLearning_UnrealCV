...
def create_cnn(observation_dimensions):
    ...
    # Load VGG16 model
    vgg16_model = VGG16(input_shape=observation_dimension[1:], weights='imagenet', include_top=False)
...
curr_observation = tf.reshape(curr_observation, (1, *curr_observation.shape))
print(curr_observation.shape)
print(curr_observation.dtype)
curr_featrues = cnnDecoder(curr_observation)
curr_featrues = tf.squeeze(curr_featrues, axis=0)
stacked_features = None
stacked_features = stack_frames(stacked_features, curr_featrues, stack_size)
# ------ train ------
...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        reshaped_stacked_features = stacked_features.reshape(1, *stacked_features.shape)
        ...
        if terminal or (t == steps_per_epoch - 1):
            last_value = 0 if done else critic(stacked_features.reshape(1, *stacked_features.shape))

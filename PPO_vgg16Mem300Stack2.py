...
def train_policy(
    ...
    with tf.GradientTape() as tape:  # Record operations for automatic differentiation.
        ...
        for obs_batch in get_batches(observation_buffer, batch_size):
            output_batch = actor(obs_batch)
            observation_outputs.append(output_batch)
        observation_outputs = tf.concat(observation_outputs, axis=0)
        
        ratio = tf.exp(
            logprobabilities(observation_outputs, action_buffer)
    ...
    for obs_batch in get_batches(observation_buffer, batch_size):
        output_batch = actor(obs_batch)
        new_observation_outputs.append(output_batch)
    new_observation_outputs = tf.concat(new_observation_outputs, axis=0)

    kl = tf.reduce_mean(
        logprobability_buffer
        - logprobabilities(new_observation_outputs, action_buffer)
    )
    kl = tf.reduce_sum(kl)
    return kl
...
load_dir_path = 'tmpVggMem300Stack2/checkpoints'
dir_path = 'tmpVggMem300Stack2/checkpoints'
f = open('PPO_records_vggMem300Stack2.txt', 'a')
